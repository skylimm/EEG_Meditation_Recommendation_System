...
# =========================
ss = st.session_state
ss.setdefault("stream_found", False)
ss.setdefault("recording_in_progress", False)
ss.setdefault("recording_finished", False)
ss.setdefault("search_completed", False)
ss.setdefault("meditation_selected", False)
ss.setdefault("post_meditation_recording_completed", False)
ss.setdefault("post_stream_found", False)
ss.setdefault("post_recording_in_progress", False)
ss.setdefault("post_recording_finished", False)
ss.setdefault("after_eeg_file", None)
ss.setdefault("before_eeg_file", None)

# Meditation timer states
ss.setdefault("meditation_running", False)
ss.setdefault("meditation_paused", False)
ss.setdefault("meditation_completed", False)
ss.setdefault("meditation_start_ts", None)
ss.setdefault("meditation_elapsed", 0)
ss.setdefault("meditation_total", 1 * 60)  # NOTE: set to 15*60 for full; kept 1*60 for quick testing
ss.setdefault("audio_played", False)
    ...
    if folder_path is None:
        ...
        with pre_tab_live:
            ...
            if ss.get("stream_found") and 'stream_process' in ss:
                ...
                # stream not yet started — allow pressing start after connect
                if not ss.get("recording_finished"):
                    pass  # wait for connection

        # ---------- Upload File (from app_1.py) ----------
        with pre_tab_upload:
